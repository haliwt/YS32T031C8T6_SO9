...
void Heat_Process(void)
{
     // static uint8_t first_compare = 1;   // ç¬¬ä¸€æ¬¡æ¯”è¾ƒæ ‡å¿—
      ...
      // -----------------------------
    // 2. ç¬¬ä¸€æ¬¡æ¯”è¾ƒï¼šå¿…é¡»ç«‹å³å†³å®š PTC å¼€å…³
    // -----------------------------
	  if(first_temp_compare_f == 0){
		...
		if(temperature >= target_temp){
            ...
            PTC_heat_open_f = 1;   // ç«‹å³æ‰“å¼€
            first_temp_compare_f = 1;         // ä»¥åè¿›å…¥æ»åæ§åˆ¶
        }
        return;
		...
		// -----------------------------
		// 3. ç¬¬äºŒæ¬¡åŠä»¥åï¼šä½¿ç”¨ -2Â°C æ»åæ§åˆ¶
		// -----------------------------
		if(first_temp_compare_f == 1)
		{
			// å½“å‰æ˜¯å¼€å¯çŠ¶æ€ â†’ é«˜äºè®¾å®šæ¸©åº¦åˆ™å…³é—­
			if(temperature >= target_temp){
					PTC_heat_open_f = 0;
			}
			else
			{
				// å½“å‰æ˜¯å…³é—­çŠ¶æ€ â†’ ä½äºè®¾å®šæ¸©åº¦ - 2 æ‰é‡æ–°æ‰“å¼€
